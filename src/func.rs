...
impl Compete for Jeremiah {
    ...
    async fn driver(&mut self) {
        loop {
            ...
            let left_voltage: f64 = (forward + turn) * Motor::MAX_VOLTAGE;
            let right_voltage: f64 = (forward - turn) * Motor::MAX_VOLTAGE;
            ...
            self.motor_left_back.set_voltage(right_voltage).ok();

            self.motor_right_front.set_voltage(left_voltage).ok();
